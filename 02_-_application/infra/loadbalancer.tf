...
resource "google_compute_global_address" "external_lb_address" {
  ...
  name         = var.load_balancer_address_name
  ip_version   = "IPV4"
...
resource "google_compute_managed_ssl_certificate" "external_lb_managed_ssl_cert" {
  project     = local.project_id
  name        = var.load_balancer_managed_cert_name
...
resource "google_compute_region_network_endpoint_group" "external_lb_neg" {
  project               = local.project_id
  name                  = "${var.load_balancer_prefix}-managed-neg"
  region                = local.region
...
resource "google_compute_backend_service" "backend_service" {
  project     = local.project_id
  name        = "${var.load_balancer_prefix}-managed-backend"
  protocol    = "HTTP"
  port_name   = "http"
  timeout_sec = 30
...
resource "google_compute_url_map" "lb_url_map" {
  project         = local.project_id
  name            = "${var.load_balancer_prefix}-managed-url-map"
  default_service = google_compute_backend_service.backend_service.id
}

resource "google_compute_target_https_proxy" "lb_target_proxy" {
  project          = local.project_id
  name             = "${var.load_balancer_prefix}-target-proxy"
...
resource "google_compute_global_forwarding_rule" "lb_fwd_rule" {
  project    = local.project_id
  name       = "${var.load_balancer_prefix}-fwd-rule"
...
resource "google_compute_url_map" "https_redirect" {
  project = local.project_id
  name    = "${var.load_balancer_prefix}-https-redirect"
